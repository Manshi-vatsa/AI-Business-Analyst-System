...
if exist "%MAVEN_PROJECTBASEDIR%\.mvn\jvm.config" (
  ...
  @for /f "tokens=1,2 delims==" %%a in ("%MAVEN_PROJECTBASEDIR%\.mvn\jvm.config") do (
    if "%%a"=="JAVA_OPTS" set JAVA_OPTS=%%b
  )
  @endlocal
)

@REM ==== START CORE MAVEN ====
...
@REM Find Maven from the environment variable if set
...
if "%OS%" == "Windows_NT" set "MAVEN_HOME=%MAVEN_HOME:\=/%"
